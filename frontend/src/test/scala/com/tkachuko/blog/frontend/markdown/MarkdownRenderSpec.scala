...
object MarkdownRenderSpec extends TestSuite {
  ...
  val tests = TestSuite {
    ...
    "not change plain HTML" - {
      ...
      Algorithm.partition(html, Italic).render ==> html
    }
    ...
    "render content with italic" - {
      ...
      Algorithm.partition(markdown, Italic).render ==>
        """
    ...
    "render content with bold text" - {
      ...
      Algorithm.partition(markdown, Bold).render ==>
        """
    ...
    "render headings up to 6" - {
      ...
      Algorithm.partition(h1md, H(1)).render ==>
        """
          |<h1>h</h1>
        """.stripMargin

      Algorithm.partition(h2md, H(2)).render ==>
        """
          |<h2>h</h2>
        """.stripMargin

      Algorithm.partition(h3md, H(3)).render ==>
        """
          |<h3>h</h3>
        """.stripMargin

      Algorithm.partition(h4md, H(4)).render ==>
        """
          |<h4>h</h4>
        """.stripMargin
    }
    ...
    "render inline code block" - {
      ...
      Algorithm.partition(markdown, InlineCode).render ==>
        """
    ...
    "not change previous post created in html" - {
      ...
      Algorithm.partition(post, InlineCode).render ==> post
      Algorithm.partition(post, Language("scala")).render ==> post
      Algorithm.partition(post, Language("java")).render ==> post
      Algorithm.partition(post, H(1)).render ==> post
      Algorithm.partition(post, H(2)).render ==> post
    }
    ...
    "correctly partition markdown string with code" - {
      ...
      Algorithm.partition(markdown, Language("scala")).render ==> rendered
    }
  }
}
